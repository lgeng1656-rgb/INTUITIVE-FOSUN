...
test("阶段入口使用手动播放的视频介绍页并允许点击空白返回", async () => {
  ...
  assert.match(source, /page\.copy\.map/);
  assert.match(source, /className="video-intro-copy-line"/);
...
test("homepage copy uses auxiliary decision and the requested postoperative order", async () => {
  const source = await readFile(new URL("./App.jsx", import.meta.url), "utf8");

  assert.doesNotMatch(source, />术中决策</);
  assert.match(source, />辅助决策</);
  assert.ok(source.indexOf(">辅助决策<") < source.indexOf(">远程教学<"));
  assert.ok(source.indexOf(">手术复盘<") < source.indexOf(">质控管理<"));
});
